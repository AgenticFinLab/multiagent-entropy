"""
Executable test script for Debate MAS (multi-agent with majority voting).

- Parses YAML config via PyYAML (yaml.safe_load)
- Command line option `-c/--config` points to the YAML config file
- Instantiates `DebateMAS` (from debate.py)
- Loads questions from the dataset defined in config and runs inference
- Run: `python examples/uTEST/test_debate_entropy_infer.py -c configs/uTEST/debate_entropy_infer.yml`
...
- configs/uTEST/debate_entropy_infer.yml
"""
...
def main():
    """A demo to test the DebateMAS with a YAML config file."""
    load_dotenv()

    parser = argparse.ArgumentParser(
        description="Run debate multi-agent test with YAML config."
    )
    parser.add_argument(
        "-c",
        "--config",
        type=str,
        default="configs/uTEST/debate_entropy_infer.yml",
    ...
    agent = DebateMAS(run_config=run_config)
    ...
    for start_idx in range(0, total_samples, batch_size):
        ...
        # Look for the last occurrence of the orchestrator result
        for res in reversed(final_state["agent_results"]):
        ...
        if orchestrator_outputs:
            for i, out in enumerate(orchestrator_outputs):
                print(f"Sample {start_idx + i} Final Answer:", out)
        else:
