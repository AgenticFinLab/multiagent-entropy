"""
Executable test script for Hybrid MAS (two-layer with loop and full context).

- Parses YAML config via PyYAML (yaml.safe_load)
- Command line option `-c/--config` points to the YAML config file
- Instantiates `OrchestratorHybrid` (from hybrid.py)
- Loads questions from the dataset defined in config and runs inference
- Run: `python examples/uTEST/test_hybrid_entropy_infer.py -c configs/uTEST/hybrid_entropy_infer.yml`
...
- configs/uTEST/hybrid_entropy_infer.yml
"""
...
def main():
    """A demo to test the OrchestratorHybrid with a YAML config file."""
    load_dotenv()

    parser = argparse.ArgumentParser(
        description="Run hybrid multi-agent test with YAML config."
    )
    parser.add_argument(
        "-c",
        "--config",
        type=str,
        default="configs/uTEST/hybrid_entropy_infer.yml",
    ...
    agent = OrchestratorHybrid(run_config=run_config)
