...
class TempDataLoader(DataLoader):
    ...
    def load_experiment_config(
        ...
        logger.info(
            f"Using fallback config for {experiment_name}: {default_config}"
        )
    ...
    def load_entropy_tensor(
        ...
        traces_path = (
            self.results_path / dataset.lower() / model_name
            / experiment_name / "traces"
        )
    ...
    def _parse_agent_type_from_aggregated(
        ...
        if model_name:
            ...
            model_dirs = [
                d for d in aggregated_dataset_path.iterdir() if d.is_dir()
            ]
