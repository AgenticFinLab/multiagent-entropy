...
class GroundtruthLoader:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        """
        self.data_dir = (
            Path(data_dir) if data_dir is not None else Path("experiments/data")
        )
        self._cache: Dict[str, Dict[str, str]] = {}
        self._question_cache: Dict[str, Dict[str, str]] = {}
    ...
    def _load_data_file(self, task_type: str) -> Dict[str, str]:
        ...
        try:
            ...
            for sample in data:
                ...
                if main_id is not None and question is not None:
                    question_map[str(main_id)] = str(question)

            self._cache[task_type] = groundtruth_map
            self._question_cache[task_type] = question_map
            return groundtruth_map
        except Exception as e:
            print(f"Warning: Failed to load data file {file_path}: {e}")
            self._cache[task_type] = {}
            self._question_cache[task_type] = {}
            return {}
    ...
    def get_question(self, task_type: str, sample_id: str) -> Optional[str]:
        """Get question for a sample.
        ...
        """
        self._load_data_file(task_type)

        main_id = self._extract_main_id(sample_id)
        if main_id is None:
            return None

        if task_type in self._question_cache:
            return self._question_cache[task_type].get(main_id)
