...
class Aggregator:
    ...
    def extract_sample_level_data(
        ...
        base_model_entropy = self._extract_base_model_entropy(entropy_data, metrics_data)
        ...
        for model_name, model_entropy in entropy_data.get("models", {}).items():
            for exp_name, exp_entropy in model_entropy.get("experiments", {}).items():
                ...
                for sample_id, sample_metrics in samples_metrics.items():
                    ...
                    sample_round_total_tokens = defaultdict(int)
                    sample_round_total_entropy = defaultdict(float)
                    ...
                    for round_num in sorted(sample_round_total_tokens.keys()):
                        ...
                        total_entropy_round = float(sample_round_total_entropy.get(round_num, 0.0))
                    ...
                    if sample_round_total_tokens:
                        ...
                        first_density = sample_round_entropy_per_token.get(first_round, 0.0)
                        last_density = sample_round_entropy_per_token.get(last_round, 0.0)
                        sample_round_dynamic_features[
                    ...
                    for round_num in round_agent_entropy_data.keys():
                        key_mean = (
                            f"sample_round_{round_num}_mean_agent_mean_entropy"
                        )
                        key_total = (
                            f"sample_round_{round_num}_mean_agent_total_entropy"
                        )
                    ...
                    if mean_agent_mean_entropy_by_round:
                        sorted_rounds = sorted(mean_agent_mean_entropy_by_round.keys())
                        first_round = sorted_rounds[0]
                        last_round = sorted_rounds[-1]
                        first_mean = mean_agent_mean_entropy_by_round[first_round]
                        last_mean = mean_agent_mean_entropy_by_round[last_round]
                        ...
                        ] = (last_mean - first_mean) / float(num_round_steps)

                        mean_values = [
                            mean_agent_mean_entropy_by_round[r] for r in sorted_rounds
                        ]
                    ...
                    if mean_agent_total_entropy_by_round:
                        sorted_rounds = sorted(mean_agent_total_entropy_by_round.keys())
                        first_round = sorted_rounds[0]
                        last_round = sorted_rounds[-1]
                        first_total_mean = mean_agent_total_entropy_by_round[first_round]
                        last_total_mean = mean_agent_total_entropy_by_round[last_round]
                        sample_round_trend_features[
                        ...
                        ] = (last_total_mean - first_total_mean) / float(num_round_steps)
